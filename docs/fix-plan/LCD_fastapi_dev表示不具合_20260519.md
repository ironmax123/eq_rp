...
5. `fastapi dev` の起動方法によって import 解決の基準が変わり、同一ディレクトリの `LCD1602.py` を通常 import で見つけられない可能性がある。
6. FastAPI を起動している Python 環境に `smbus2` が入っておらず、`LCD1602.py` の import 時点で失敗している。
...
6. LCD 初期化成功時にもログを出す。
7. `LCD1602.py` は `api/main.py` と同一ディレクトリの実ファイルパスから直接ロードし、カレントディレクトリや package import 状態に依存しないようにする。
8. API 用の依存関係ファイルを追加し、`smbus2` を明示する。
